...
class ValidationPlotsGenerator:
    ...
    def __init__(self, mode: str = 'kinematic', config_path: Optional[Path] = None):
        ...
        # Project root 
        self.project_root = Path(__file__).parent.parent  # Go up to repo root
    ...
    def load_metadata(self) -> dict:
        """Load metadata from YAML config."""
        try:
            # Load the full config file to get metadata
            if self.config_manager.consolidated_config.exists():
                config_file = self.config_manager.consolidated_config
            elif self.mode == 'kinematic' and self.config_manager.kinematic_config.exists():
                config_file = self.config_manager.kinematic_config
            elif self.mode == 'kinetic' and self.config_manager.kinetic_config.exists():
                config_file = self.config_manager.kinetic_config
            else:
                return {}
    ...
    def generate_filters_by_phase_plots(self, tasks: Optional[List[str]] = None) -> List[str]:
        ...
        # Load metadata for timestamps and dataset info
        ...
        for task_name in tasks_to_process:
            ...
            try:
                # Create the plot with metadata for tracking
